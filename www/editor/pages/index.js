...
const graph_editor_1 = require("@fbp/graph-editor");
const evaluator_1 = require("@fbp/evaluator");
...
const allDefinitions = [
    ...evaluator_1.mathDefinitions,
    ...evaluator_1.uiDefinitions,
    ...evaluator_1.coreDefinitions,
    ...evaluator_1.netDefinitions,
];
const uiDefinitions = [
    {
        context: 'ui',
...
function Home() {
    const [selectedExample, setSelectedExample] = (0, react_1.useState)(exampleNames[0]);
    ...
    return (<div className="h-screen w-screen flex flex-col">
      <div className="h-12 bg-slate-800 border-b border-slate-700 flex items-center px-4 gap-4">
        ...
        <select value={selectedExample} onChange={(e) => setSelectedExample(e.target.value)} className="bg-slate-700 text-slate-200 text-sm rounded px-3 py-1.5 border border-slate-600 focus:outline-none focus:ring-2 focus:ring-blue-500">
          {exampleNames.map((name) => (<option key={name} value={name}>
              {name}
            </option>))}
        </select>
      ...
      <div className="flex-1">
        <graph_editor_1.GraphEditor key={selectedExample} graph={graph} definitions={graph.definitions} evaluateFn={evaluator_1.evaluate}/>
      </div>
    </div>);
}
