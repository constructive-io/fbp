...
const core_definitions_1 = require("../__fixtures__/core-definitions");
describe('core nodes', () => {
    describe('core/json/select', () => {
        it('should extract a value by simple path', async () => {
            ...
            const result = await (0, evaluate_1.evaluate)(graph, {
                definitions: core_definitions_1.coreDefinitions,
                outputNode: 'select',
                outputPort: 'value'
            });
    ...
    describe('core/json/object', () => {
        it('should build an object from inputs', async () => {
            ...
            const result = core_definitions_1.jsonObjectDef.impl({ email: 'test@example.com', name: 'Alice' }, {});
            expect(result).toEqual({
                value: { email: 'test@example.com', name: 'Alice' }
            });
        });
        it('should handle empty inputs', async () => {
            const result = core_definitions_1.jsonObjectDef.impl({}, {});
            expect(result).toEqual({ value: {} });
        });
    });
    describe('core/flow/guard', () => {
        it('should pass when ok is true', async () => {
            const result = core_definitions_1.flowGuardDef.impl({ ok: true, error: null }, {});
            expect(result).toEqual({
                pass: true,
                fail: false,
                error: null
        ...
        it('should fail when ok is false', async () => {
            const result = core_definitions_1.flowGuardDef.impl({ ok: false, error: { message: 'Something went wrong' } }, {});
            expect(result).toEqual({
                pass: false,
                fail: true,
        ...
        it('should provide default error message when none given', async () => {
            const result = core_definitions_1.flowGuardDef.impl({ ok: false }, {});
            expect(result).toEqual({
                pass: false,
                fail: true,
    ...
    describe('core/string/template', () => {
        it('should substitute placeholders', async () => {
            const result = core_definitions_1.stringTemplateDef.impl({ token: 'abc123' }, { template: 'Bearer {{token}}' });
            expect(result).toEqual({ value: 'Bearer abc123' });
        });
        it('should handle multiple placeholders', async () => {
            const result = core_definitions_1.stringTemplateDef.impl({ first: 'Hello', second: 'World' }, { template: '{{first}} {{second}}!' });
            expect(result).toEqual({ value: 'Hello World!' });
        });
        it('should keep placeholder if value is missing', async () => {
            const result = core_definitions_1.stringTemplateDef.impl({ first: 'Hello' }, { template: '{{first}} {{missing}}' });
            expect(result).toEqual({ value: 'Hello {{missing}}' });
        });
        it('should return empty string for empty template', async () => {
            const result = core_definitions_1.stringTemplateDef.impl({}, { template: '' });
            expect(result).toEqual({ value: '' });
        });
    });
    describe('core/string/concat', () => {
        it('should concatenate with prefix', async () => {
            const result = core_definitions_1.stringConcatDef.impl({ value: 'token123' }, { prefix: 'Bearer ', suffix: '' });
            expect(result).toEqual({ value: 'Bearer token123' });
        });
        it('should concatenate with suffix', async () => {
            const result = core_definitions_1.stringConcatDef.impl({ value: 'hello' }, { prefix: '', suffix: '!' });
            expect(result).toEqual({ value: 'hello!' });
        });
        it('should concatenate with both prefix and suffix', async () => {
            const result = core_definitions_1.stringConcatDef.impl({ value: 'world' }, { prefix: 'Hello, ', suffix: '!' });
            expect(result).toEqual({ value: 'Hello, world!' });
        });
    });
});
