 ...
 */
async function evaluate(graph, options) {
     ...
     * Fully async to support async node implementations.
     */
    async function evaluateNode(nodeName) {
        // Check cache first
        ...
        if (node.type === 'graphInput') {
            ...
            const inputName = portNameProp?.value || nodeName;
        ...
        if (node.type === 'graphProp') {
            ...
            const propName = propNameProp?.value || nodeName;
            ...
            if (node.outputs) {
                for (const outputPort of node.outputs) {
                    ...
                    const outputBoundaryNode = node.nodes.find(n => {
                        ...
                        return portNameProp?.value === outputPort.name;
                    });
