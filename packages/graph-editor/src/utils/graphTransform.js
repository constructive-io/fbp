...
exports.deriveBoundaryProps = deriveBoundaryProps;
exports.cwdToPath = cwdToPath;
exports.isRootCwd = isRootCwd;
exports.getParentCwd = getParentCwd;
exports.getChildCwd = getChildCwd;
exports.getNodesAtScope = getNodesAtScope;
exports.getEdgesAtScope = getEdgesAtScope;
exports.findNodeAtPath = findNodeAtPath;
exports.updateNodesAtScope = updateNodesAtScope;
exports.updateEdgesAtScope = updateEdgesAtScope;
exports.ensureDerivedPorts = ensureDerivedPorts;
exports.migrateLegacyGraph = migrateLegacyGraph;
exports.prepareForStorage = prepareForStorage;
 ...
 */
function deriveBoundaryProps(nodes) {
    return nodes
        .filter(n => n.type === types_1.BOUNDARY_NODE_TYPES.prop)
        .map(n => {
        const propName = (0, types_1.getPortNameFromBoundary)(n) || n.name;
        const propType = (0, types_1.getDataTypeFromBoundary)(n);
        const defaultValue = (0, types_1.getDefaultFromBoundary)(n);
        return {
            name: propName,
            type: propType,
 ...
 * "/subnet1", "subnet2" -> "/subnet1/subnet2"
 */
function getChildCwd(cwd, nodeName) {
 ...
 */
function getNodesAtScope(graph, cwd) {
    if (isRootCwd(cwd))
        return graph.nodes;
 ...
 */
function getEdgesAtScope(graph, cwd) {
    if (isRootCwd(cwd))
        return graph.edges;
    const path = cwdToPath(cwd);
    let currentNode;
 ...
 */
function findNodeAtPath(graph, path) {
    if (path.length === 0)
        return null;
    let nodes = graph.nodes;
    let node;
 ...
 */
function updateNodesAtScope(graph, cwd, updater) {
    if (isRootCwd(cwd)) {
        return { ...graph, nodes: updater(graph.nodes) };
    }
    const path = cwdToPath(cwd);
    const updateRecursive = (nodes, remainingPath) => {
 ...
 */
function updateEdgesAtScope(graph, cwd, updater) {
    if (isRootCwd(cwd)) {
 ...
 */
function ensureDerivedPorts(graph) {
 ...
 */
function ensureDerivedPortsOnNode(node) {
 ...
 * - Port/prop names stored as properties: { name: 'portName', value: 'a' }
 */
function migrateLegacyGraph(graph) {
    // Check if we need to migrate (has ports but no boundary nodes)
    const hasBoundaryNodes = graph.nodes.some(n => n.type === types_1.BOUNDARY_NODE_TYPES.input ||
        n.type === types_1.BOUNDARY_NODE_TYPES.output ||
        n.type === types_1.BOUNDARY_NODE_TYPES.prop);
    if (hasBoundaryNodes) {
    ...
    (graph.props || []).forEach((prop, i) => {
        ...
        boundaryNodes.push({
            ...
            type: types_1.BOUNDARY_NODE_TYPES.prop,
...
function migrateLegacyNode(node) {
    ...
    const hasBoundaryNodes = node.nodes.some(n => n.type === types_1.BOUNDARY_NODE_TYPES.input ||
        n.type === types_1.BOUNDARY_NODE_TYPES.output ||
        n.type === types_1.BOUNDARY_NODE_TYPES.prop);
    if (hasBoundaryNodes) {
 ...
 */
function prepareForStorage(graph) {
